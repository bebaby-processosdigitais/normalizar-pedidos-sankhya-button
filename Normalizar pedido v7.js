// ============================================================
//  NORMALIZADOR DE PEDIDO - v7 (MARKETPLACE + SITE)
...
//
//  ============ O QUE MUDOU DA v6 ============
//  A v6 so tratava marketplace (TOP 1755) e recusava o resto.
//  A v7 trabalha com PERFIS: cada origem de pedido tem suas
//  regras, e o script detecta qual aplicar.
//
//    MARKETPLACE  detectado por CODTIPOPER 1755
//    SITE         detectado por CODVEND 12 (ABC) ou 26 (KIKKABOO)
//
//  O calculo de venda, KP, Pix e arredondamento e o MESMO nos
//  dois. O que muda e o cabecalho - ver PERFIS abaixo.
//
...
// ============================================================
//  PERFIS
//  Cada origem de pedido tem suas regras de cabecalho. O calculo
//  de valor, KP e arredondamento e identico nos dois.
// ============================================================

var CODTIPOPER_MARKETPLACE = 1755;

// CODVEND que identificam pedido vindo do site
var VENDEDORES_SITE = { "12": "SITE ABC", "26": "SITE KIKKABOO" };

// TOPs esperadas no site - so gera aviso, nao recusa
var TOPS_SITE = { "1722": true, "1728": true };

var PERFIS = {
  MARKETPLACE: {
    nome: "MARKETPLACE",
    codcencus: 20000000,          // null = nao altera
    tipfrete: true,               // aplica a regra de Extra nota / Incluso
    volumes: true,                // grava QTDVOL
    pixPorCanal: true,            // regra por AD_CANAL_MKTPLACE
    pixAbsorveSempre: false,
    obsInterna: "OPERADOR"        // "NOME - dd/mm"
  },
  SITE: {
    nome: "SITE",
    codcencus: null,              // ja vem correto - NAO mexer
    tipfrete: false,              // nao mexer
    volumes: true,
    pixPorCanal: false,
    pixAbsorveSempre: true,       // desconto do rodape sempre entra na base
    obsInterna: "SITE"            // "LOJA \n #pedido \n NOME - dd/mm"
  }
};

var CAMPO_PEDIDO_EXT    = "AD_PEDIDOMKTPLACE";
...
function lerCabecalho(nunota) {
    ...
    c.CODVEND     = Number(vo.asBigDecimalOrZero("CODVEND"));
    ...
    c.PEDEXT_OK = false;
    try { c.PEDEXT = String(vo.asString(CAMPO_PEDIDO_EXT)); c.PEDEXT_OK = true; }
    catch (e) { c.PEDEXT = null; }
    if (c.PEDEXT === null || c.PEDEXT === "null") c.PEDEXT = "";
...
// Site tem prioridade: o CODVEND e mais especifico que a TOP.
function detectarPerfil(cab) {
    var lojaSite = VENDEDORES_SITE[String(cab.CODVEND)];
    if (lojaSite !== undefined) {
        var p = PERFIS.SITE;
        p.loja = lojaSite;
        if (TOPS_SITE[String(cab.CODTIPOPER)] === undefined) {
            avisos.push("Pedido de site com TOP " + cab.CODTIPOPER +
                        " (esperadas 1722 ou 1728). Conferir antes de gravar.");
        }
        return p;
    }
    if (cab.CODTIPOPER === CODTIPOPER_MARKETPLACE) {
        var m = PERFIS.MARKETPLACE;
        m.loja = null;
        return m;
    }
    return null;
...
// Observacao Interna conforme o perfil.
// No site o operador COMPLETA depois (transportadora, parcelamento,
// ID da transacao), entao o script grava so o cabecalho do texto.
function montarObsInterna(perfil, cab) {
    var assinatura = nomeUsuario() + " - " + ddmm();
    if (perfil.obsInterna === "SITE") {
        var linhas2 = [perfil.loja];
        if (cab.PEDEXT !== "") linhas2.push("#" + cab.PEDEXT);
        linhas2.push("");
        linhas2.push(assinatura);
        return linhas2.join("\n");
    }
    return assinatura;
}
...
// ---------- regra de Pix por perfil ----------

function decidirPix(cab, perfil) {
    ...
    if (!temDesc) {
        return { pix: 0, origem: "sem desconto no rodape", erro: null };
    }

    // SITE: o desconto do rodape SEMPRE entra na base
    if (perfil !== null && perfil.pixAbsorveSempre) {
        return { pix: (cab.VLRDESCTOT !== 0 ? round2(cab.VLRDESCTOT) : null),
                 origem: perfil.nome + ": desconto entra na base", erro: null };
    }
...
function calcular(cab, itens, perfil) {
    var D = { produtos: [], kpExist: [], perfil: perfil };
    for (var i = 0; i < itens.length; i++) {
    ...
    // Pix
    var dpix = decidirPix(cab, D.perfil);
    D.pix = dpix.pix;
    if (D.pix === null) {
    ...
    D.obsInterna = montarObsInterna(perfil, cab);
...
function validar(cab, perfil) {
    if (perfil === null) {
        erros.push("Origem nao reconhecida: TOP " + cab.CODTIPOPER +
                   ", CODVEND " + cab.CODVEND + ". Este botao trata pedido de " +
                   "MARKETPLACE (TOP " + CODTIPOPER_MARKETPLACE + ") e de SITE " +
                   "(CODVEND 12 ou 26).");
        return;
    }
    var dp = decidirPix(cab, perfil);
...
function relatorio(cab, D, nunota, fin) {
    box("SITUACAO ATUAL");
    rel.push("  PERFIL ............ " + D.perfil.nome +
             (D.perfil.loja ? "  (" + D.perfil.loja + ")" : ""));
    rel.push("  TOP / negociacao .. " + cab.CODTIPOPER + " / " + cab.CODTIPVENDA);
    rel.push("  Vendedor .......... " + cab.CODVEND);
    if (D.perfil.pixPorCanal) {
        rel.push("  Canal ............. " + (cab.CANAL === "" ? "(vazio)" : cab.CANAL));
    }
    if (D.perfil.obsInterna === "SITE") {
        rel.push("  Pedido externo .... " + (cab.PEDEXT === "" ? "(vazio)" : cab.PEDEXT));
    }
    ...
    if (D.acaoKp === "INSERIR" || D.acaoKp === "ATUALIZAR") {
        rel.push("  (gabarito buscado na TOP " + cab.CODTIPOPER + " - campos como");
        rel.push("   RESERVA sao dirigidos pela operacao)");
    }

    box("CABECALHO");
    rel.push("  Dt. neg / mov ..... " + cab.DTNEG + " / " + cab.DTMOV + "  ->  " + ddmm());
    rel.push("  Centro resultado .. " + cab.CODCENCUS + "  ->  " +
             (D.perfil.codcencus === null ? "(perfil " + D.perfil.nome +
              " nao altera)" : String(D.perfil.codcencus)));
    ...
    if (obsAcao.indexOf("\n") >= 0) {
        rel.push("  Obs. Interna ...... " + cab.OBSINT + "  ->");
        var partes = obsAcao.split("\n");
        for (var oi = 0; oi < partes.length; oi++) {
            rel.push("       | " + partes[oi]);
        }
        rel.push("       (o operador completa o resto: transportadora,");
        rel.push("        parcelamento, ID da transacao)");
    } else {
    ...
    rel.push("  Qtd. volumes ...... " + cab.QTDVOL + "  ->  " +
             (D.perfil.volumes ? String(D.qtdVol) : "(perfil nao altera)"));
    ...
    var tf;
    if (!D.perfil.tipfrete) {
        tf = "(perfil " + D.perfil.nome + " nao altera)";
    } else if (cab.VLRFRETE === 0 && TIPFRETE_EXTRANOTA !== null) {
...
function gravarCabecalho(cab, D, nunota) {
    var reg = linhas[0], ts = tsHoje(), P = D.perfil, oque = [];
    ...
    oque.push("datas, observacao, descontos zerados");

    if (P.codcencus !== null) {
        reg.setCampo("CODCENCUS", bd(P.codcencus));
        oque.push("centro " + P.codcencus);
    }
    if (P.volumes) {
        reg.setCampo("QTDVOL", bd(D.qtdVol));
        oque.push("volumes " + D.qtdVol);
    }
    ...
    if (P.tipfrete) {
        if (cab.VLRFRETE === 0 && TIPFRETE_EXTRANOTA !== null) {
            ...
            oque.push("TIPFRETE " + TIPFRETE_EXTRANOTA);
        } else if (cab.VLRFRETE !== 0 && TIPFRETE_INCLUSO !== null) {
            reg.setCampo("TIPFRETE", TIPFRETE_INCLUSO);
            oque.push("TIPFRETE " + TIPFRETE_INCLUSO);
        }
    }
    reg.save();
    oque.push(podeObs ? "obs. interna" : "obs. interna PRESERVADA");
    feitos.push("cabecalho [" + P.nome + "]: " + oque.join(", "));
}
...
// O gabarito TEM de ser da MESMA TOP do pedido. Campos como RESERVA e
// ATUALESTOQUE sao dirigidos pela operacao: a TGFTOP mostra ATUALEST = 'R'
// na 1722 e na 1755, mas 'B' na 1728. Gabarito de outra TOP dispara
//    ORA-20101: Reserva diferente da definicao na TOP (TRG_INC_TGFITE)
// Cobertura verificada: 1722, 1728 e 1755 tem as 7 faixas de KP.
function gabaritoKp(sku, nunota, codtipoper) {
    var q = getQuery();
...
function gravarKp(D, nunota, codtipoper) {
    ...
    var gab = gabaritoKp(D.kp.sku, nunota, codtipoper);
    if (gab === null) {
        throw "sem linha de KP do SKU " + D.kp.sku + " (sem desconto) em outro " +
              "pedido da TOP " + codtipoper + " para copiar os campos. " +
              "O gabarito precisa ser da MESMA TOP - campos como RESERVA sao " +
              "dirigidos pela operacao. Insira um KP dessa faixa a mao uma vez " +
              "nessa TOP e repita.";
    }
...
function verificar(cab, D, nunota) {
    ...
    if (GRAVAR_CABECALHO) {
        if (D.perfil.codcencus !== null && c2.CODCENCUS !== D.perfil.codcencus) {
            f.push("CODCENCUS " + c2.CODCENCUS);
        }
        if (D.perfil.volumes && c2.QTDVOL !== D.qtdVol) f.push("QTDVOL " + c2.QTDVOL);
...
function main() {
    ...
    rel.push("NORMALIZADOR DE PEDIDO - v7 (marketplace + site)");
    ...
    var perfil = detectarPerfil(cab);
    if (itens.length === 0) erros.push("Pedido sem itens.");
    else validarConsistencia(itens);
    validar(cab, perfil);

    var D = (erros.length === 0) ? calcular(cab, itens, perfil) : null;
    ...
    try {
        ...
        if (GRAVAR_KP)        gravarKp(D, nunota, cab.CODTIPOPER); else rel.push("  (KP off)");
    ...
    var msg = "Pedido " + nunota + " normalizado [" + D.perfil.nome +
              (D.perfil.loja ? " - " + D.perfil.loja : "") + "].\n" +
